...
const paymentCategory = computed<"card" | "mobile">({
...
function clearDetails() {
  (Object.keys(details) as (keyof typeof details)[]).forEach(
    (key) => (details[key] = ""),
  );
}
...
function formatCardNumber(event: Event) {
  const card = event.target as HTMLInputElement;
  details.number = card.value
    .replace(/\D/g, "")
    .slice(0, 19)
    .replace(/(.{4})(?=.)/g, "$1 ");
  card.value = details.number;
}
</script>
<template>
