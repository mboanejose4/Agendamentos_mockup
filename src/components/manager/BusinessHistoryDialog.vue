...
import { state, money, dateLabel } from "@/stores/applicationStore.ts";
const props = defineProps<{
  subject?: { type: "client" | "staff"; id: string; name: string } | null;
}>();
...
<template>
  <AppModal
    ...
    ><p class="mb-4 text-small text-muted">
      {{ plural(rows.length, "marcação", "marcações") }} neste estabelecimento
    </p>
