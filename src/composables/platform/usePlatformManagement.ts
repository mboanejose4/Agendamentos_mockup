...
/** Formulário de criação e edição de um estabelecimento, com o provisionamento
    do gestor incluído. */
export interface CompanyForm extends Omit<
  Business,
...
/** Formulário de conta, com os campos que só existem para profissionais. */
export interface UserForm extends Omit<User, "passwordHash" | "passwordSalt"> {
  password: string;
  title: string;
  serviceIds: string[];
  staffId: string;
...
export function usePlatformManagement() {
  ...
  /* Formulário do estabelecimento, com os campos extra do provisionamento. */
  ...
  function openCompany(item: Business | null = null): void {
    Object.assign(
      companyForm,
      item
        ? {
  ...
  function performRemoval() {
    ...
    if (!pending) return;
    if (pending.collection === "users" && pending.item.id === state.userId) {
      removalError.value = "Não pode eliminar a conta que está a utilizar.";
      return;
    }
    const result = removeRecord(pending.collection, pending.item.id);
    if (!result.ok) {
  ...
  function toggleCompany(item: Business): void {
  ...
  function openUser(item: User | null = null): void {
  ...
  function toggleUser(item: User): void {
  ...
  const validationIssues = computed(() => {
    ...
    /* O vínculo utilizador–empresa é o centro do modelo: uma conta ligada a um
       estabelecimento que já não existe é uma inconsistência, não um detalhe. */
    const exists = (businessId: string): boolean =>
      state.db.businesses.some((venue) => venue.id === businessId);
    for (const item of state.db.users) {
      if (item.businessId && !exists(item.businessId))
        issues.push({
          id: `${item.id}-user-business`,
          text: `A conta de ${item.name} está associada a um estabelecimento que já não existe.`,
        });
      if (
        item.staffId &&
        !state.db.staff.some((entry) => entry.id === item.staffId)
        ...
        issues.push({
          id: `${item.id}-user-staff`,
          text: `A conta de ${item.name} aponta para um profissional que já não existe.`,
        });
    }
    const scoped: [
      string,
      { id: string; businessId: string; name?: string; code?: string }[],
    ][] = [
      ["O profissional", state.db.staff],
      ["O serviço", state.db.services],
      ["O recurso", state.db.resources],
      ["A promoção", state.db.promotions],
    ];
    for (const [label, records] of scoped)
      for (const item of records)
        if (!exists(item.businessId))
          issues.push({
            id: `${item.id}-business`,
            text: `${label} ${item.name || item.code} não tem estabelecimento associado.`,
          });
    return issues;
  });
  function exportLogs() {
  ...
  const ticketPriorityNames: Record<TicketPriority, string> = {
    low: "Baixa",
    normal: "Normal",
    high: "Alta",
    urgent: "Urgente",
  ...
  function openConversation(item: Ticket): void {
  ...
  function changeTicketStatus(status: TicketStatus): void {
    saveRecord("tickets", {
      ...selectedTicket.value,
      status,
