...
import BookingServiceStep from "@/Component/bookings/BookingServiceStep.vue";
import BookingTimeStep from "@/Component/bookings/BookingTimeStep.vue";
import BookingReviewStep from "@/Component/bookings/BookingReviewStep.vue";
import BookingSummary from "@/Component/bookings/BookingSummary.vue";
import BookingPaymentDialog from "@/Component/bookings/BookingPaymentDialog.vue";
import { provideBookingFlow } from "@/Composable/bookings/bookingContext.js";
const {
  state,
  go,
  money,
...
<template>
  <div v-if="current" class="mx-auto max-w-[1050px]">
    ...
    <section v-else class="mx-auto mt-[10px] mb-[40px] max-w-[600px] py-5 text-center md:mt-[40px]">
      <span
      ...
      <div class="flex flex-wrap justify-center gap-[15px] py-[25px] md:gap-5">
        ...
        ><span class="flex items-center gap-2 text-caption text-primary-text"
          ><AppIcon name="receipt" class="w-[17px]" />{{ created?.id }}</span
        >
      </div>
      ...
      <div class="form-actions justify-center">
        <button class="btn btn-secondary w-full md:w-auto" @click="downloadCalendar">
          <AppIcon name="download" :size="17" />Adicionar ao
          calendário</button
        ><button class="btn btn-primary w-full md:w-auto" @click="go('appointments')">
