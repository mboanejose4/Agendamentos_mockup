<script setup>
import BusinessCard from "@/Component/discovery/BusinessCard.vue";
  ...
  <div>
    ...
    >
      ...
      <button
        ...
        type="submit"
      >
        <AppIcon name="search" :size="18" /><span class="max-sm:hidden">Pesquisar</span>
      </button>
    </form>
    <div
    ...
    <section>
      ...
      >
        <div>
          ...
          <p class="mt-1.5 text-caption text-muted">
            ...
            <span class="px-[7px] text-muted">·</span> Escolha o seu próximo momento
          </p>
        </div>
        <div class="flex items-center gap-3">
          ...
          ><button class="btn btn-secondary btn-compact" @click="filtersOpen = true">
      ...
      <div v-else class="empty-state">
        <AppIcon :name="state.view === 'favorites' ? 'heart' : 'search'" :size="36" />
        <h3>
          {{
            state.view === "favorites"
              ? "Ainda sem favoritos"
        ...
        >
          {{
            state.view === "favorites" ? "Explorar estabelecimentos" : "Limpar filtros"
          }}
        </button>
      </div>
    </section>
    ...
    <AppModal v-model="filtersOpen" title="Filtrar estabelecimentos"
      ><form @submit.prevent="filtersOpen = false">
        ...
        ><label class="check-field"
          ><input type="checkbox" v-model="onlineOnly" />Aceita pagamento online</label
        >
        <div class="form-actions">
          <button type="button" class="btn btn-secondary" @click="clearFilters">
            Limpar</button
          ><button class="btn btn-primary">Ver {{ results.length }} resultados</button>
        </div>
      </form></AppModal
    >
  </div>
