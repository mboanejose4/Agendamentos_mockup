...
import { useBusinessDiscovery } from "@/Composable/discovery/useBusinessDiscovery.js";
const {
  state,
  go,
  money,
...
<template>
  <div v-if="current">
    <button class="text-button -mt-[5px] mb-[22px] text-caption" @click="go('explore')">
      <AppIcon name="arrow-left" :size="17" />Voltar a explorar
    </button>
    <div class="relative mb-[35px] h-[220px] rounded-lg bg-surface-muted max-sm:h-[190px]">
      <img
        :src="current.image"
        :alt="current.name"
        ...
        <template v-if="detailTab === 'Serviços'"
          ...
          >
            <AppIcon name="tag" :size="20" /><span class="flex-1"
              ><strong class="block text-caption"
                ...
                momento</strong
              ><small class="mt-1 block text-caption">Cupão {{ promotions[0].code }}</small></span
            ><button
              class="icon-btn"
              title="Copiar cupão"
          ...
          >
            ...
            <div class="flex flex-col items-end gap-2.5">
              ...
              ><button class="btn btn-secondary btn-compact" @click="book(item.id)">
      ...
      >
        ...
        <div class="flex gap-[13px] py-[17px] text-muted">
          <AppIcon name="clock" /><span
            ><strong class="text-caption text-ink [overflow-wrap:anywhere]">{{ current.opens }} - {{ current.closes }}</strong
            ><small class="mt-[5px] block text-caption text-muted">Horário de funcionamento</small></span
          >
        </div>
        <div class="flex gap-[13px] py-[17px] text-muted">
          <AppIcon name="map-pin" /><span
            ><strong class="text-caption text-ink [overflow-wrap:anywhere]">{{ current.city }}</strong
            ><small class="mt-[5px] block text-caption text-muted">{{ current.address }}</small></span
          >
        </div>
        <a
          ...
          ><AppIcon name="phone" /><span
            ><strong class="text-caption text-ink [overflow-wrap:anywhere]">{{ current.phone }}</strong
            ><small class="mt-[5px] block text-caption text-muted">Contactar estabelecimento</small></span
          ></a
        ><a
          class="flex gap-[13px] py-[17px] text-muted"
          :href="`mailto:${current.email}`"
          ><AppIcon name="mail" /><span
            ><strong class="text-caption text-ink [overflow-wrap:anywhere]">{{ current.email }}</strong
            ><small class="mt-[5px] block text-caption text-muted">E-mail</small></span
          ></a
        >
        <div class="flex items-center gap-[9px] border-t border-line pt-5 text-caption text-muted">
