...
import { salons, services, professionals, initialBookings, notifications } from './data/mockData'
...
export const helpers = {
  ...
  professionalById: (id) => professionals.find((item) => item.id === Number(id)),
}

export const currentSalon = computed(() => helpers.salonById(appStore.selectedSalonId) || salons[0])
...
export function startBooking(salonId, serviceId = null) {
  ...
  appStore.booking = { ...defaultBooking(), salonId: Number(salonId), serviceId }
  go('booking')
}
...
export function markNotificationsRead() {
  appStore.notifications.forEach((item) => { item.read = true })
...
export function addBooking() {
  ...
  const booking = {
    ...
    status: 'Confirmada',
    payment: appStore.booking.paymentMethod === 'online' ? 'Pago online' : 'Pagamento no salão',
