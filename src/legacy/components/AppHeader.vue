    ...
    <div class="shell header-main">
      <button class="brand" @click="go('home')" aria-label="Ir para o início">
        ...
        <span class="brand-copy"
          ><strong>Agenda Serviços</strong
          ><small>Marque. Gerencie. Cresça.</small></span
        >
      </button>
      <nav
        class="desktop-nav"
        v-if="store.role === 'guest' || store.role === 'client'"
      >
        <button :class="{ active: store.view === 'home' }" @click="go('home')">
          Início
        </button>
        <button
          :class="{ active: store.view === 'explore' }"
          @click="go('explore')"
        >
          Explorar
        </button>
        <button
          v-if="store.role === 'client'"
          :class="{ active: store.view === 'client-dashboard' }"
          @click="go('client-dashboard')"
        >
          Minha conta
        </button>
      </nav>
      <div class="header-actions">
        ...
        <button
          class="btn btn-primary compact desktop-only"
          v-if="store.role === 'guest'"
        ...
        >
          ...
          <span class="profile-copy"
            ...
            ><small>Alterar perfil</small></span
          >
        </button>
      </div>
    </div>
...
<script>
import { appStore as store, go } from "../store.js";
