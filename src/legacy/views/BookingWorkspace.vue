...
const people = computed(() =>
  state.db.staff.filter(
    (p) =>
      p.active &&
      p.businessId === draft.businessId &&
      p.serviceIds.includes(draft.serviceId),
  ),
);
...
const isRestaurant = computed(() => current.value?.category === "Restauração");
...
function confirm() {
  ...
  if (
    ...
    error.value = "Este horário já não está disponível. Escolha outro horário.";
    step.value = 2;
    return;
  }
...
function downloadCalendar() {
  ...
  const content = [
    ...
    `DTSTART:${stamp(start)}`,
    `DTEND:${stamp(end)}`,
    `SUMMARY:${escape(selectedService.value.name)}`,
...
<template>
  <div v-if="current" class="booking-workspace">
    <button
      v-if="step < 4"
    ...
    <div v-if="step < 4" class="booking-layout">
      <section class="booking-form-section">
        <template v-if="step === 1"
          ...
          ><template v-if="people.length"
            ><h3>Com quem prefere?</h3>
            <div class="staff-options">
              <button
                :class="['staff-option', { selected: !draft.staffId }]"
                @click="draft.staffId = ''"
              >
                <span class="avatar"><AppIcon name="users" /></span
                ><span
                  ><strong>Sem preferência</strong
                  ><small>Primeiro profissional disponível</small></span
                ><AppIcon
                  :name="!draft.staffId ? 'circle-check' : 'plus'"
                  :size="18"
                /></button
              ><button
                ...
                :class="[
                  'staff-option',
                  { selected: draft.staffId === person.id },
                ]"
                @click="draft.staffId = person.id"
              >
                <span class="avatar">{{
                  person.name
                    .split(" ")
                ...
                ><span
                  ...
                  ><small>{{ person.title }}</small></span
                ><AppIcon
                  v-if="draft.staffId === person.id"
                  name="circle-check"
                  :size="18"
                />
              </button></div></template
        ...
        <template v-if="step === 2"
          ...
          <div class="booking-date-grid">
            <button
              v-for="date in dates"
              :key="date"
              :class="{ selected: draft.date === date }"
          ...
          <div class="section-heading">
            ...
            <span class="muted">{{ selectedService?.duration }} minutos</span>
          </div>
          <div class="time-slot-grid">
            <button
              v-for="time in slots"
              :key="time"
              :class="{ selected: draft.time === time }"
          ...
          <div v-if="!slots.length" class="empty-state">
            ...
            <p>
              Escolha outra data ou ajuste a preferência de profissional ou
              recurso.
            </p>
          </div></template
        >
        <template v-if="step === 3"
          ...
          <div class="review-details">
            <div>
              <span>Estabelecimento</span><strong>{{ current.name }}</strong>
            </div>
            <div>
              <span>Serviço</span><strong>{{ selectedService?.name }}</strong>
            </div>
            <div>
              <span>Data e hora</span
              ><strong>{{ dateLabel(draft.date) }} · {{ draft.time }}</strong>
            </div>
            <div>
              <span>{{ isRestaurant ? "Pessoas" : "Profissional" }}</span
              ><strong>{{
                isRestaurant
                  ? draft.partySize
          ...
          <label class="field"
            >Pedido especial <span class="muted">(opcional)</span
            ><textarea
              v-model="draft.notes"
              rows="2"
          ...
          <p v-else-if="appliedCoupon" class="success-message">
            Cupão {{ appliedCoupon }} aplicado: menos
            {{ money(totals.discount) }}.
          </p>
          <h3>Como prefere pagar?</h3>
          <label
            class="payment-choice"
            :class="{ selected: draft.paymentMethod === 'onsite' }"
            ><input
              type="radio"
              v-model="draft.paymentMethod"
              value="onsite"
            /><AppIcon name="wallet" /><span
              ><strong>No estabelecimento</strong
              ><small>Pague no momento do atendimento.</small></span
            ></label
          ><label
            v-if="
            ...
            "
            class="payment-choice"
            :class="{ selected: draft.paymentMethod === 'online' }"
            ><input
              type="radio"
              v-model="draft.paymentMethod"
              value="online"
          ...
          >
          <p class="policy-note">
            <AppIcon name="calendar-clock" :size="16" />Pode cancelar ou
            reagendar até {{ current.cancelHours }} horas antes.
          </p></template
        >
      ...
      <aside class="booking-summary">
        ...
        <div class="booking-summary-body">
          ...
          <h3>{{ current.name }}</h3>
          <p><AppIcon name="map-pin" :size="14" />{{ current.city }}</p>
          ...
          <div v-if="draft.time" class="summary-service">
            <strong>{{ dateLabel(draft.date) }}</strong
            ><span>{{ draft.time }}</span>
          </div>
          <div v-if="appliedCoupon && !totals.error" class="summary-service">
            <span>Desconto</span><strong>−{{ money(totals.discount) }}</strong>
          </div>
          <div class="summary-total">
            <span>Total</span><strong>{{ totalLabel }}</strong>
          </div>
        </div>
        <p class="summary-footnote">
          <AppIcon name="shield-check" :size="16" />Os seus planos, bem
          organizados.
        </p>
      </aside>
    </div>
    <section v-else class="booking-confirmed">
      ...
      <div class="confirmation-info">
        ...
        ><span><AppIcon name="clock" />{{ created?.time || draft.time }}</span
        ><span><AppIcon name="receipt" />{{ created?.id }}</span>
      </div>
      <p class="muted">
        {{
      ...
      <div class="form-actions">
        <button class="btn secondary" @click="downloadCalendar">
          <AppIcon name="download" :size="17" />Adicionar ao calendário</button
        ><button class="btn primary" @click="go('appointments')">
    ...
    <AppModal v-model="paymentOpen" title="Pagamento de teste"
      ><div class="gateway-summary">
        <AppIcon name="credit-card" :size="30" /><span
          >Total a pagar<strong>{{ totalLabel }}</strong></span
        >
      </div>
      <p class="local-note">
      ...
      <label class="field"
        >Método<select v-model="paymentMethod">
      ...
      <div class="gateway-actions">
        <button class="btn primary" @click="finish('paid')">
          <AppIcon name="check" :size="17" />Aprovar pagamento de teste</button
        ><button class="btn secondary" @click="finish('pending')">
          Manter pagamento pendente</button
        ><button class="btn secondary" @click="paymentState = 'declined'">
          Testar recusa</button
        ><button
          class="text-button"
          @click="
            paymentOpen = false;
