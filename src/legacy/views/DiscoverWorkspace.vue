...
const people = computed(() =>
  state.db.staff.filter((p) => p.businessId === current.value?.id && p.active),
);
...
<template>
  <div v-if="state.view !== 'business'" class="discover-page">
    <header class="discovery-header">
      <div>
        <span class="eyebrow">UM TEMPO SÓ SEU</span>
        <h1>
          {{
    ...
    <form class="discovery-search" @submit.prevent>
  ...
  >
    ...
    <section class="discovery-results">
      <div class="section-heading">
        <div>
          ...
          <p>
            ...
            <span class="separator-dot">·</span> Escolha o seu próximo momento
          </p>
        </div>
        <div class="results-tools">
          <select v-model="sort" aria-label="Ordenar estabelecimentos">
            <option value="recommended">Melhor avaliação</option>
            <option value="price">Menor preço</option>
            <option value="name">Nome</option></select
          ><button class="btn secondary compact" @click="filtersOpen = true">
      ...
      <div v-if="results.length" class="business-grid">
        <article
          ...
          <div class="business-photo">
            ...
            >
              <img
                ...
                :alt="company.name"
                @error="photoError"
              /><span class="photo-fallback">{{ company.name }}</span></button
            ><span class="photo-category"
              ><AppIcon
                :name="selectedCategory(company.category).icon"
                :size="13"
              />{{ company.category }}</span
            ><button
              :class="[
                'favorite-button',
                { selected: favorites.includes(company.id) },
              ]"
              :aria-label="
          ...
          <div class="business-card-body">
            <div class="business-rating">
              <span
                ...
                <small>({{ company.reviewCount || 0 }})</small></span
              ><span class="subtle-tag">{{ company.city }}</span>
            </div>
            <h3>
              <button @click="openCompany(company)">{{ company.name }}</button>
            </h3>
            <p class="business-address">
              <AppIcon name="map-pin" :size="14" />{{ company.address }}
            </p>
            <div class="business-service-tags">
              <span
                v-for="item in servicesFor(company.id).slice(0, 2)"
                :key="item.id"
                >{{ item.name }}</span
              >
            </div>
            <div class="business-card-footer">
              <span
                >A partir de
                <strong>{{ money(minPrice(company.id)) }}</strong></span
              ><button
                class="round-link"
    ...
    <section class="discovery-bottom">
      <div class="editorial-note">
        <span class="category-icon peach"
          ><AppIcon name="calendar-check" :size="24"
        /></span>
        <div>
          <h3>O seu dia merece espaço.</h3>
          <p>
  ...
  <div v-else-if="current" class="business-detail-page">
    <button class="text-button back-button" @click="go('explore')">
      <AppIcon name="arrow-left" :size="17" />Voltar a explorar
    </button>
    <div class="business-detail-cover">
      <img :src="current.image" :alt="current.name" @error="photoError" /><span
        :class="['category-icon', selectedCategory(current.category).color]"
        ><AppIcon :name="selectedCategory(current.category).icon" :size="28"
      /></span>
    </div>
    <header class="page-header business-heading">
      <div>
        <span class="eyebrow"
          >{{ current.category }} <span class="separator-dot">·</span>
          {{ current.city }}</span
        >
        <h1>{{ current.name }}</h1>
        <p>
      ...
      <button class="btn secondary" @click="favorite(current.id)">
    ...
    <div class="business-detail-layout">
      <section>
        <div class="tabs">
          <button
            v-for="tab in ['Serviços', 'Equipa', 'Informações']"
            :key="tab"
        ...
        <template v-if="detailTab === 'Serviços'"
          ><div v-if="promotions.length" class="promotion-strip">
            <AppIcon name="tag" :size="20" /><span
              ><strong
                >{{ promotions[0].discount }}% para o seu próximo
                momento</strong
              ><small>Cupão {{ promotions[0].code }}</small></span
            ><button
              class="icon-btn"
              title="Copiar cupão"
          ...
          <div class="section-heading">
            ...
            <span class="muted"
              >{{ servicesFor(current.id).length }} serviços</span
            >
          </div>
          <article
          ...
          >
            ...
            <div class="service-choice-action">
              <strong>{{ money(item.price) }}</strong
              ><button class="btn secondary compact" @click="book(item.id)">
        ...
        >
        <div v-else-if="detailTab === 'Equipa'" class="team-grid">
          <article
          ...
          >
            ...
            <span class="badge success"
              >{{ person.serviceIds.length }} serviços</span
            >
          </article>
          <p v-if="!people.length" class="muted">
            As reservas são atribuídas aos recursos disponíveis do
            estabelecimento.
          </p>
        </div>
        <div v-else class="business-information">
      ...
      <aside class="business-info-aside">
        <h3>Encontre o seu momento</h3>
        <p>{{ current.description }}</p>
        <div class="info-line">
          <AppIcon name="clock" /><span
            ><strong>{{ current.opens }} - {{ current.closes }}</strong
        ...
        <div class="info-line">
          <AppIcon name="map-pin" /><span
            ...
            ><small>{{ current.address }}</small></span
          >
        </div>
        <a class="info-line" :href="`tel:${current.phone}`"
          ><AppIcon name="phone" /><span
            ><strong>{{ current.phone }}</strong
            ><small>Contactar estabelecimento</small></span
          ></a
        ><a class="info-line" :href="`mailto:${current.email}`"
          ><AppIcon name="mail" /><span
            ><strong>{{ current.email }}</strong
            ><small>E-mail</small></span
          ></a
        >
        <div class="business-payment-note">
