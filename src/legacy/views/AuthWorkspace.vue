...
<template>
  <div v-if="state.view === 'onboard'" class="onboarding-page">
    <header class="page-header">
      <div>
        <span class="eyebrow">A SUA EMPRESA NO ELO</span>
        <h1>Vamos conhecer o seu negócio.</h1>
        <p>Comece pelos dados do estabelecimento.</p>
      </div>
    </header>
    <form class="onboarding-form" @submit.prevent="createCompany">
      <div class="form-grid">
        <label class="field"
        ...
        ><label class="field"
          >Categoria<select v-model="company.category">
        ...
        ><label class="field"
          >Cidade<input v-model.trim="company.city" required /></label
        ><label class="field"
          >Endereço<input
        ...
        ><label class="field"
          >Abertura<input v-model="company.opens" type="time" required /></label
        ><label class="field"
          >Encerramento<input v-model="company.closes" type="time" required
        /></label>
      </div>
      <label class="field"
        >Sobre o estabelecimento<textarea
      ...
      <div class="form-actions">
        <button type="button" class="btn secondary" @click="go('explore')">
          Voltar</button
  ...
  <div v-else class="auth-layout">
    <section class="auth-introduction">
      <span class="category-icon mint"
        ><AppIcon name="calendar-check" :size="30" /></span
      ><span class="eyebrow">BEM-VINDO AO ELO</span>
      <h1>O seu tempo.<br />Os seus planos.<br />Tudo ligado.</h1>
      <p>Há sempre um bom momento para cuidar de si.</p>
      <img
    ...
    <section class="auth-form-area">
      <div
        ...
        class="company-registration-note"
        role="status"
      >
        <AppIcon name="building-2" :size="20" />
        <p>
          <strong>Registar a sua empresa</strong><br />Primeiro, crie uma conta
          ou inicie sessão. A seguir, preencha os dados do estabelecimento.
        </p>
      </div>
      <div class="tabs">
        <button
      ...
      <form @submit.prevent="submit">
        ...
        ></label>
        <p v-if="error" class="error-message" role="alert">{{ error }}</p>
        <button :disabled="busy" class="btn primary full">
      ...
      <button
        v-if="state.returnView !== 'onboard'"
        type="button"
        class="text-button company-registration-link"
        @click="
          state.returnView = 'onboard';
          mode = 'register';
          error = '';
      ...
      <div class="auth-local-info">
        ...
        <p>
          Conta guardada neste navegador. Use dados de teste neste ambiente
          local.
        </p>
      </div>
