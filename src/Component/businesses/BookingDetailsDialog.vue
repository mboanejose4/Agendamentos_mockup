  ...
  <AppModal v-model="detailOpen" title="Detalhes da reserva">
    <template v-if="selectedBooking"
      ><div>
        ...
        <dl class="mb-5 grid grid-cols-1 gap-x-6 gap-y-3 text-caption sm:grid-cols-2">
          <div>
          ...
          <div v-if="selectedBooking.partySize > 1">
            ...
            <dd class="font-medium text-ink">{{ selectedBooking.partySize }}</dd>
          </div>
        </dl>
        ...
        >
          <div>
            <strong class="block text-body-lg text-ink"
              >{{ money(selectedBooking.total) }}</strong
