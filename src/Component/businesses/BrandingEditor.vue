...
import { normalizeBrand, textOn, isHexColor } from "@/Utils/theme.js";
import BusinessIcon from "@/Component/ui/BusinessIcon.vue";
const props = defineProps({
  modelValue: Object,
  businessName: { type: String, default: "A sua empresa" },
});
...
async function upload(event) {
  ...
  try {
    ...
    uploadError.value = error.message || "Não foi possível ler esta imagem.";
  } finally {
    event.target.value = "";
  }
}
...
<template>
  <fieldset class="my-6 min-w-0 rounded-xl border border-line bg-surface p-4 sm:p-6">
      ...
      <label class="field"
        ...
        <div class="flex items-center gap-2.5">
          <input
            ...
            @input="update('primaryColor', $event.target.value)"
          /><input
            ...
            maxlength="7"
            :value="brand.primaryColor"
            @change="update('primaryColor', $event.target.value)"
            required
          /></div></label
      ><label class="field"
        >Cor secundária
        <div class="flex items-center gap-2.5">
          <input
            ...
            @input="update('secondaryColor', $event.target.value)"
          /><input
            ...
            maxlength="7"
            :value="brand.secondaryColor"
            @change="update('secondaryColor', $event.target.value)"
            required
          /></div
      ></label>
    </div>
    ...
    >
      <BusinessIcon
        ...
        :name="businessName"
        :size="48"
      /><strong class="min-w-[100px] flex-1 break-words">{{
