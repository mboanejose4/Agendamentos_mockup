...
import { makeSeed, shiftDate } from '../src/core/seed.js'

const storage = new Map()
globalThis.localStorage = { getItem: (key) => storage.get(key) || null, setItem: (key, value) => storage.set(key, value), removeItem: (key) => storage.delete(key) }
const { state, availableSlots, createBooking, updateBooking, cancelBooking, markPaid, bookingTotal, registerAccount, loginAccount, logout, removeRecord, setAccountPassword } = await import('../src/core/state.js')
...
const draft = (patch = {}) => ({ businessId: 'b1', serviceId: 's1', staffId: 'p1', date: shiftDate(2), time: '10:00', clientId: 'u1', paymentMethod: 'onsite', ...patch })

test('availability respects full duration, staff eligibility, blocks and occupied intervals', () => {
  state.db.blocks.push({ id: 'block-test', businessId: 'b1', staffId: 'p1', date: shiftDate(2), start: '12:00', end: '13:00' })
...
test('restaurant reservations assign tables by capacity and allow independent concurrent tables', () => {
  const tableDraft = draft({ businessId: 'b4', serviceId: 's10', staffId: '', time: '13:00', partySize: 4 })
...
test('discounts persist in totals, reject expired or cross-business coupons, and online payments start pending', () => {
  const result = createBooking(draft({ coupon: ' bemvindo10 ', paymentMethod: 'online' }))
  ...
  const stored = JSON.parse(storage.get('elo.agendamento.v3'))
...
test('cancellation enforces client notice and records refunds while reopening availability', () => {
  const result = createBooking(draft({ paymentMethod: 'online', paymentStatus: 'paid' }))
  ...
  assert.equal(updateBooking(result.record.id, { status: 'confirmed' }).ok, false)
})
...
test('deactivating a service preserves the appointment history and prevents new bookings', () => {
  ...
  assert.equal(state.db.services.find((entry) => entry.id === 's1').active, false)
  assert.equal(state.db.bookings.length, 1)
...
test('accounts can register and sign in with a salted hash, without storing plaintext passwords', async () => {
  ...
  const result = await registerAccount({ name: 'Cliente Teste', email: 'TEST@example.com', phone: '+258 840000001', password })
  assert.equal(result.ok, true)
  ...
  assert.equal(storage.get('elo.agendamento.v3').includes(password), false)
  assert.equal((await registerAccount({ name: 'Repetido', email: 'test@example.com', password })).ok, false)
  logout()
  assert.equal(state.role, 'guest')
  assert.equal((await loginAccount({ email: 'test@example.com', password: 'wrong-password' })).ok, false)
  assert.equal((await loginAccount({ email: 'test@example.com', password })).ok, true)
  assert.equal(state.userId, result.record.id)
  assert.equal((await setAccountPassword(result.record.id, 'new-strong-secret')).ok, true)
  assert.equal((await loginAccount({ email: 'test@example.com', password })).ok, false)
  assert.equal((await loginAccount({ email: 'test@example.com', password: 'new-strong-secret' })).ok, true)
})
